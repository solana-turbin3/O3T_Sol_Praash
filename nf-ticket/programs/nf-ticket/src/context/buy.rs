...
use anchor_spl::{
    ...
    metadata::{
        mpl_token_metadata::instructions::{
            TransferCpi, TransferCpiAccounts, TransferInstructionArgs, UnlockCpi,
            UnlockCpiAccounts, UnlockInstructionArgs,
        },
...
use mpl_token_metadata::types::{Creator, TransferArgs, UnlockArgs};
...
impl<'info> Buy<'info> {
    pub fn buy(&mut self, bumps: BuyBumps) -> Result<()> {
        // Pay for the NFT
        ...
        if self.metadata.seller_fee_basis_points != 0 && self.metadata.creators.is_some() {
            ...
            for (i, _creator) in creators.iter().enumerate() {
                ...
                require_keys_eq!(
                    ...
                    InstrospectionError::InvalidTokenProgram
                );
                require_eq!(ix.data[0], 2u8, InstrospectionError::InvalidIx);
                require!(
                    ix.data[4..12].eq(&creator_amount.to_le_bytes()),
                    InstrospectionError::InvalidAmount
                );
                require_keys_eq!(
                    ix.accounts.get(1).unwrap().pubkey,
                    _creator.address,
                    InstrospectionError::InvalidCreator
                );
            }
        }
        ...
        let seed = &[b"listing", marketplace_key.as_ref(), &[bumps.listing]];
